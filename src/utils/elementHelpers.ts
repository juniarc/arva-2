...
import { avaibleCategories } from "@/services/fixed-data/fixedData";
...
export const calculateAverageRatingShop = (products: any) => {
  const validRatings = products.map((product: any) =>
    product.ratings === "number" && !isNaN(product.ratings)
      ? product.ratings
      : 0
  );
...
export const convertCategoryNameToId = (categoryName: string) => {
  const category = avaibleCategories.find(
...
// export const convertCartResponseToCartItems = async (
//   cartResponse: ResponseGetCartUser
// ) => {
//   try {
...
//       if (!productDetail) {
//         throw new Error(
//           `Product detail not found for product_id: ${cartItem.product_id}`
//         );
//       }
//       const selectedVariant = productDetail.variant.find(
//         (item: any) => item.variant_id === cartItem.variant_id
//       );
//       if (!selectedVariant) {
//         throw new Error(
//           `Variant not found for variant_id: ${cartItem.variant_id}`
//         );
//       }
...
export const separateCartItemsByShop = (
  cartItems: CartItem[]
): Record<number, { shop_name: string; products: CartItem[] }> => {
...
export const calculateRatings = (products: Product[]) => {
  ...
  const totalRating = products.reduce(
    (sum, product) => sum + (Number(product.ratings) || 0),
    0
  );
